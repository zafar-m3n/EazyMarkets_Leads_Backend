...
const importLeads = async (req, res) => {
  try {
    ...
    const [statuses, sources] = await Promise.all([LeadStatus.findAll(), LeadSource.findAll()]);
    ...
    leads.forEach((row, idx) => {
      ...
      const isEmptyRow =
        !firstName && !lastName && !company && !email && !phoneRaw && !country && !status && !source && !noteBody;
  ...
  try {
    return res.json({
      ...
      required_rules: [
        ...
        "Company, country, source, status, and notes do not count as lead identifiers.",
      ],
      notes: [
